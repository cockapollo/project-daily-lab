...
async function summaryHandler(req, res) {
  ...
  const cached = locationCache.get(city);
  const displayName = cached ? cached.name : city;
  ...
  try {
    ...
    res.end(JSON.stringify({ city: displayName, record_count: history.length, summary }));
  } catch (err) {
